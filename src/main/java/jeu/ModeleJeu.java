...
public class ModeleJeu extends Observable {
	...
	private Carte matrice;              // matrice de jeu
	...
	public ModeleJeu() {
		...
		this.matrice = new Carte(20, 30);

		// On remplit la liste de monde
		// this.mondes.add(MondeSnow.Snow());
		this.mondes.add(MondeDemonstration.Demo());
	...
	public void chargerNiveau() {
		...
		Niveau courant = this.mondes.get(this.indiceMondeCourant).getNiveau(this.indiceNiveauCourant);
		/* On construit la matrice du niveau */
		this.construireMatrice(courant);
		/* On charge le heros */
		Coordonnees c = new Coordonnees(courant.posHeros().getX(), courant.posHeros().getY());
		this.ajouterHeros(new Heros(c, "/ressources/Images/hero_fry_stop_left.png", 2));
		/* On charge les MadMonks */
		for (Coordonnees cour : courant.recupererPosMadMonk()) {
			c = new Coordonnees(cour.getX(), cour.getY());
			this.ajouterMadMonk(new MadMonk(c, "/ressources/Images/mad_monk_stop_left.png", 1, this.matrice, 0),
					getHeros());
		}
	 ...
	 */
	private void construireMatrice(Niveau niveauCourant) {
		/* initialisation de la carte avec la taille des tableaux */
		this.matrice = new Carte(20, 30);
		...
		Positionnable.GESTION_ANIM.setMatrice(this.matrice);
	}
	...
	public void rebootNiveau() {
		fini = true;
		Positionnable.GESTION_ANIM.finirAnimation();
		try {
			Thread.sleep(200);
		...
		this.personnages = new ArrayList<Personnage>();
		this.LodeRunner = new Thread(new ThreadJoueur());
		Positionnable.GESTION_ANIM = new ThreadAnimation();
	 ...
	 */
	public Bloc getEnv(Coordonnees pos) {
	 ...
	 * 
	 * @param pos
	 *            coordonnees ou l'on souhaite recuperer le bloc
	 * @return le bloc situe a cette position
	 */
	public List<Monde> getMondes() {
		return this.mondes;
	}
	/**
	 ...
	 */
	public Item getObj(Coordonnees pos) {
	...
	public void creuser() {
		...
		if (this.getHeros().isRegardeAGauche()) {
			/* Creuser a gaucher */
			c = new Coordonnees(this.getHeros().getPosition().getX() - 1, this.getHeros().getPosition().getY() + 1);
	...
	public void utiliserGadget(char keyChar) {
		try {
			...
			switch (keyChar) {
			case 'x':
				posItem = new Coordonnees(this.getHeros().getPosition().getX(), this.getHeros().getPosition().getY());
			...
			case 'p':
				if (getHeros().isRegardeAGauche()) {
					posItem = new Coordonnees(this.getHeros().getPosition().getX() - 1,
...
//				System.out.println("GAZZZ");
				...
				if (getHeros().isRegardeAGauche()) {
					/* Gaz a gauche */
					posItem = new Coordonnees(this.getHeros().getPosition().getX() - 3,
...
//			System.out.println("GGGGGGGGG");
			...
			Positionnable.GESTION_ANIM.ajouterPositionnableAnime(this.sortie, Images.gate);
	...
	private class ThreadJoueur implements Runnable {
		...
		public void run() {
			...
			while (!fini && getHeros().estVivant()) {
				...
				// Update animations
				Positionnable.GESTION_ANIM.activerAnimation();

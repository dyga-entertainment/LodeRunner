...
public class ImagesMadMonk {
    ...
    public ImagesMadMonk() {
        ...
        for (int i = 0; i < moveRight.length; i++) {
            moveRight[i] = get("/ressources/Images/mad_monk_walk" + (i + 1) + "_right.png");
            moveLeft[i] = get("/ressources/Images/mad_monk_walk" + (i + 1) + "_left.png");
        }
        for (int i=0; i < fall.length; i++) {
            fall[i] = get("/ressources/Images/mad_monk_fall" + (i + 1) + ".png");
            climbLadder[i] = get("/ressources/Images/mad_monk_ladder" + (i + 1) + ".png");
        }
    }

package MVC.Model.personnage;

import java.util.Random;

import MVC.Model.bloc.Bloc;
import MVC.Model.bloc.BlocBarreTraversee;
import MVC.Model.bloc.BlocEchelle;
import MVC.Model.bloc.BlocTerre;
import MVC.Model.item.Item;
import MVC.Model.item.Lingot;
import MVC.Model.jeu.Carte;
...
public class MadMonk extends Personnage {
	...
	/** carte Matrice de MVC.Model.jeu dans laquelle le MadMonk evolue */
	private Carte carte;
	...
	private void chuter() {
            ...
            while (!(carte.getBloc(getPosition()) instanceof BlocEchelle)
                ...
                /* on tombe */
                if (!isMouvement()) {
                    getSprite().playSequence(Utils.helper.ImagesMadMonk.fall, true);
                    setMouvement(true);
                }
                bas();
                pauseThread(20);
	...
	private void tomber() {
            ...
            if ((this.carte.getBloc(enDessous) instanceof BlocTerre) && (!carte.getBloc(enDessous).isConcret())) {
                ...
                while ((getPixels().getY()) > (auDessus.getY() * 32)) {
                    /* on remonte */
                    if (!isMouvement()) {
                        getSprite().playSequence(Utils.helper.ImagesMadMonk.climbLadder, true);
                        setMouvement(true);
                    }
                    haut();
                    pauseThread(20);
...
//                  System.out.println("lolol3");
                    ...
                    if ((!(carte.getBloc(gauche) instanceof Bloc)) 
                         ...
                         */
                        getSprite().playSequence(Utils.helper.ImagesMadMonk.moveLeft, true);
...
//                    System.out.println("lolol4");
                    ...
                    if ((!(carte.getBloc(droite) instanceof Bloc)) 
                         ...
                         */
                        getSprite().playSequence(Utils.helper.ImagesMadMonk.moveRight, true);

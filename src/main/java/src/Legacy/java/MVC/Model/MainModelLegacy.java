...
import MVC.Model.personnage.Heros;
import MVC.Model.personnage.MadMonk;
import MVC.Model.personnage.Personnage;
...
/**
 * Classe MainModelLegacy. Classe qui represente le niveau de MVC.Model.jeu actuel. Permet de
 * gerer les interactions entre les differentes classes en MVC.Model.jeu.
...
public class MainModelLegacy extends Observable {
	...
	private MVC.Model.jeu.Carte matrice;              // matrice de MVC.Model.jeu

	private List<Personnage> personnages;
	private List<MVC.Model.jeu.Monde> mondes;
	...
	// threaddy mercury de MVC.Model.jeu utile ?
	...
	public MainModelLegacy() {
		...
		this.mondes = new ArrayList<MVC.Model.jeu.Monde>();

		// initialisation de la carte avec la taille des tableaux
		this.matrice = new MVC.Model.jeu.Carte(20, 30);
	...
	public void chargerNiveau() {
		...
		MVC.Model.jeu.Niveau courant = this.mondes.get(this.indiceMondeCourant).getNiveau(this.indiceNiveauCourant);
		/* On construit la matrice du niveau */
		this.construireMatrice(courant);
		/* On charge le heros */
		MVC.Model.jeu.Coordonnees c = new MVC.Model.jeu.Coordonnees(courant.posHeros().getX(), courant.posHeros().getY());
		//this.ajouterHeros(new Heros(c, ResourcesPaths.ANIM_PLAYER_IDLE_PATH + "hero_fry_stop_left.png", 2));
		/* On charge les MadMonks */
		for (MVC.Model.jeu.Coordonnees cour : courant.recupererPosMadMonk()) {
			c = new MVC.Model.jeu.Coordonnees(cour.getX(), cour.getY());
	 ...
	 */
	private void construireMatrice(MVC.Model.jeu.Niveau niveauCourant) {
		/* initialisation de la carte avec la taille des tableaux */
		this.matrice = new MVC.Model.jeu.Carte(20, 30);
		...
		MVC.Model.jeu.Positionnable.GESTION_ANIM.setMatrice(this.matrice);
	}
	...
	public void rebootNiveau() {
		fini = true;
		MVC.Model.jeu.Positionnable.GESTION_ANIM.finirAnimation();
		try {
			Thread.sleep(200);
		...
		this.personnages = new ArrayList<Personnage>();
		this.LodeRunner = new Thread(new ThreadJoueur());
		MVC.Model.jeu.Positionnable.GESTION_ANIM = new ThreadAnimation();

	}

    /**
     * Permet de jouer au MVC.Model.jeu.
     */
	public void BoucleDeJeu() {
	    ...
	    // Mettre a jour l'image du MVC.Model.jeu

    }

	/**
	 * startThread Permet de démarrer le modele de MVC.Model.jeu sur le niveau
	 * actuellement charge
	 */
	 ...
	 * en cours
	 * @return la liste des personnages en MVC.Model.jeu
	 */
	 ...
	 */
	public Bloc getEnv(MVC.Model.jeu.Coordonnees pos) {
	 ...
	 * @return le bloc situe a cette position
	 */
	public List<MVC.Model.jeu.Monde> getMondes() {
		return this.mondes;
	}
	/**
	 ...
	 */
	public Item getObj(MVC.Model.jeu.Coordonnees pos) {
	...
	public void creuser() {
		MVC.Model.jeu.Coordonnees c;
		if (this.getHeros().isRegardeAGauche()) {
			/* Creuser a gaucher */
			c = new MVC.Model.jeu.Coordonnees(this.getHeros().getPosition().getX() - 1, this.getHeros().getPosition().getY() + 1);
	...
	public void utiliserGadget(char keyChar) {
		try {
			MVC.Model.jeu.Coordonnees posItem;
			switch (keyChar) {
			case 'x':
				posItem = new MVC.Model.jeu.Coordonnees(this.getHeros().getPosition().getX(), this.getHeros().getPosition().getY());
			...
			case 'p':
				if (getHeros().isRegardeAGauche()) {
					posItem = new MVC.Model.jeu.Coordonnees(this.getHeros().getPosition().getX() - 1,
...
//				System.out.println("GAZZZ");
				...
				if (getHeros().isRegardeAGauche()) {
					/* Gaz a gauche */
					posItem = new MVC.Model.jeu.Coordonnees(this.getHeros().getPosition().getX() - 3,
...
//			System.out.println("GGGGGGGGG");
			...
			MVC.Model.jeu.Positionnable.GESTION_ANIM.ajouterPositionnableAnime(this.sortie, Utils.helper.Images.gate);
	...
	private void testerSiFall() {
		...
		if (getHeros().getCentre().getY() < 614) {
			MVC.Model.jeu.Coordonnees enDessous = new MVC.Model.jeu.Coordonnees(getHeros().getPosition().getX(),
	...
	private class ThreadJoueur implements Runnable {
		...
		public void run() {
			...
			while (!fini && getHeros().estVivant()) {
				...
				// Update animations
				MVC.Model.jeu.Positionnable.GESTION_ANIM.activerAnimation();
				...
				while ((up || down || left || right) && getHeros().estVivant() && !fini) {
					...
					while (left && getHeros().estVivant() && !fini) {
						...
						if (getEnv(new MVC.Model.jeu.Coordonnees(getHeros().getPosition().getX(),
						...
						if (getHeros().getCentre().getX() > 15) {
							...
							if (getHeros().getPixels().getX() < getHeros().getPosition().getX() * 32) {
								MVC.Model.jeu.Coordonnees aGauche = new MVC.Model.jeu.Coordonnees(getHeros().getPosition().getX() - 1,
					...
					while (up && getHeros().estVivant() && !fini) {
						...
						if (getHeros().getCentre().getY() > 16) {
							...
							if (getHeros().getPixels().getY() > getHeros().getPosition().getY() * 32) {
								...
								MVC.Model.jeu.Coordonnees auDessus = new MVC.Model.jeu.Coordonnees(getHeros().getPosition().getX(),
										getHeros().getPosition().getY() - 1);
								/*
					...
					while (right && getHeros().estVivant() && !fini) {
						...
						if (getHeros().getCentre().getX() < 945) {
							...
							if ((getHeros().getPixels().getX()) < getHeros().getPosition().getX() * 32) {
								MVC.Model.jeu.Coordonnees aDroite = new MVC.Model.jeu.Coordonnees(getHeros().getPosition().getX() + 1,
										getHeros().getPosition().getY());
...
//			System.out.println(fini);
			...
			if (!fini) {
				...
				MVC.Model.jeu.Coordonnees c = new MVC.Model.jeu.Coordonnees(getHeros().getPixels().getX(), getHeros().getPixels().getY() - 32);
				getHeros().setMouvement(false);
				getHeros().getSprite().setPosition(c.getX(), c.getY());
				if (getHeros().isRegardeAGauche()) {
					/* Animation a gauche */
					MVC.Model.jeu.Positionnable.GESTION_ANIM.ajouterPositionnableAnime(getHeros(), Utils.helper.Images.deathRight);
				} else {
					MVC.Model.jeu.Positionnable.GESTION_ANIM.ajouterPositionnableAnime(getHeros(), Utils.helper.Images.deathLeft);
				}
				...
				MVC.Model.jeu.Positionnable.GESTION_ANIM.ajouterPositionnableAnime(getHeros(), Utils.helper.Images.win);
				try {
					Thread.sleep(6000);
				} catch (InterruptedException e) {
					// TODO Auto-generated catch block
		...
		private void detecterObjet(MVC.Model.jeu.Coordonnees posHeros) {

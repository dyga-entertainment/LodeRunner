...
/**
 * Classe jeu.ModeleJeu. Classe qui represente le niveau de jeu actuel. Permet de
 * gerer les interactions entre les differentes classes en jeu.
...
public class ModeleJeu extends Observable {
	...
	private jeu.Carte matrice;              // matrice de jeu

	private List<Personnage> personnages;
	private List<jeu.Monde> mondes;
	...
	private bloc.BlocPortailFinNiveau sortie;
	...
	public ModeleJeu() {
		...
		this.mondes = new ArrayList<jeu.Monde>();

		// initialisation de la carte avec la taille des tableaux
		this.matrice = new jeu.Carte(20, 30);
	...
	public void chargerNiveau() {
		...
		jeu.Niveau courant = this.mondes.get(this.indiceMondeCourant).getNiveau(this.indiceNiveauCourant);
		/* On construit la matrice du niveau */
		this.construireMatrice(courant);
		/* On charge le heros */
		jeu.Coordonnees c = new jeu.Coordonnees(courant.posHeros().getX(), courant.posHeros().getY());
		this.ajouterHeros(new Heros(c, ResourcesPaths.ANIM_PLAYER_IDLE_PATH + "hero_fry_stop_left.png", 2));
		/* On charge les MadMonks */
		for (jeu.Coordonnees cour : courant.recupererPosMadMonk()) {
			c = new jeu.Coordonnees(cour.getX(), cour.getY());
	 ...
	 */
	private void construireMatrice(jeu.Niveau niveauCourant) {
		/* initialisation de la carte avec la taille des tableaux */
		this.matrice = new jeu.Carte(20, 30);
		/* positionnement des blocs */
		/* un bloc etant jeu.Positionnable, il possede ses propres coordonnees */
		for (Bloc bloci : niveauCourant.recupererListeBlocs()) {
			this.matrice.setBloc(bloci);
		}
		/* positionnement des items */
		/* un item etant jeu.Positionnable, il possede ses propres coordonnees */
		for (item.Item courant2 : niveauCourant.recupererListeItems()) {
		...
		jeu.Positionnable.GESTION_ANIM.setMatrice(this.matrice);
	}
	...
	public void rebootNiveau() {
		fini = true;
		jeu.Positionnable.GESTION_ANIM.finirAnimation();
		try {
			Thread.sleep(200);
		...
		this.personnages = new ArrayList<Personnage>();
		this.LodeRunner = new Thread(new ThreadJoueur());
		jeu.Positionnable.GESTION_ANIM = new ThreadAnimation();
	...
	public void BoucleDeJeu() {
	    ...
	    // Mettre a jour l'image du jeu

    }

	/**
	 * startThread Permet de démarrer le modele de jeu sur le niveau
	 * actuellement charge
	 */
	 ...
	 * en cours
	 * @return la liste des personnages en jeu
	 */
	 ...
	 */
	public Bloc getEnv(jeu.Coordonnees pos) {
	 ...
	 * @return le bloc situe a cette position
	 */
	public List<jeu.Monde> getMondes() {
		return this.mondes;
	}
	/**
	 ...
	 */
	public item.Item getObj(jeu.Coordonnees pos) {
	...
	public void creuser() {
		...
		if (this.getHeros().isRegardeAGauche()) {
			/* Creuser a gaucher */
			c = new jeu.Coordonnees(this.getHeros().getPosition().getX() - 1, this.getHeros().getPosition().getY() + 1);
	...
	public void utiliserGadget(char keyChar) {
		try {
			jeu.Coordonnees posItem;
			switch (keyChar) {
			case 'x':
				posItem = new jeu.Coordonnees(this.getHeros().getPosition().getX(), this.getHeros().getPosition().getY());
				((Heros) this.getHeros()).utiliserInv(new item.PetiteBombe(posItem));
				// this.matrice.getBloc(c).getSprite().playSequence(Images.explosion,false);
				item.PetiteBombe b = new item.PetiteBombe(posItem);
				this.matrice.setItem(b);
				b.utiliser();
				break;
			case 'p':
				if (getHeros().isRegardeAGauche()) {
					posItem = new jeu.Coordonnees(this.getHeros().getPosition().getX() - 1,
							this.getHeros().getPosition().getY() - 1);
				} else {
					posItem = new jeu.Coordonnees(this.getHeros().getPosition().getX() + 1,
							this.getHeros().getPosition().getY() - 1);
				}
				((Heros) this.getHeros()).utiliserInv(new item.Pioche(posItem));
				// this.matrice.getBloc(c).getSprite().playSequence(Images.explosion,false);
				item.Pioche p = new item.Pioche(posItem);
...
//				System.out.println("GAZZZ");
				...
				if (getHeros().isRegardeAGauche()) {
					/* Gaz a gauche */
					posItem = new jeu.Coordonnees(this.getHeros().getPosition().getX() - 3,
				...
				((Heros) this.getHeros()).utiliserInv(new item.Gaz(posItem));
				item.Gaz g = new item.Gaz(posItem);
...
//				System.out.println("nothing");
	...
	public void ramasserGadget() {
		// try {
		item.Item i = this.getObj(this.getHeros().getPosition());
		if (i instanceof item.Gadget) {
			item.Gadget g = (item.Gadget) i;
...
//			System.out.println("GGGGGGGGG");
			...
			jeu.Positionnable.GESTION_ANIM.ajouterPositionnableAnime(this.sortie, helper.Images.gate);
	...
	private void testerSiFall() {
		...
		if (getHeros().getCentre().getY() < 614) {
			jeu.Coordonnees enDessous = new jeu.Coordonnees(getHeros().getPosition().getX(),
			 ...
			 */
			if ((!(getEnv(getHeros().getPosition()) instanceof bloc.BlocEchelle) && ((!(getEnv(enDessous) instanceof Bloc)
					&& !(getEnv(getHeros().getPosition()) instanceof bloc.BlocBarreTraversee))
					...
					|| ((getEnv(enDessous) instanceof Bloc) && (!getEnv(enDessous).isConcret())
							&& !(getEnv(getHeros().getPosition()) instanceof bloc.BlocBarreTraversee))
							&& !(getEnv(enDessous) instanceof bloc.BlocEchelle)))) {
	...
	private class ThreadJoueur implements Runnable {
		...
		public void run() {
			...
			while (!fini && getHeros().estVivant()) {
				...
				// Update animations
				jeu.Positionnable.GESTION_ANIM.activerAnimation();
				...
				while ((up || down || left || right) && getHeros().estVivant() && !fini) {
						...
						if (getHeros().getCentre().getY() < 625) {
							...
							if (getHeros().getPixels().getY() < getHeros().getPosition().getY() * 32) {
								...
								if (!(getEnv(enDessous) instanceof Bloc)
										&& !(getEnv(getHeros().getPosition()) instanceof bloc.BlocBarreTraversee)) {
									activerAnimation(helper.Images.fall);
									getHeros().bas();
									 ...
									 */
									if (!(getEnv(getHeros().getPosition()) instanceof bloc.BlocBarreTraversee)) {
										/*
										 ...
										 * l'echelle)
										 */
										if ((getEnv(getHeros().getPosition()) instanceof bloc.BlocEchelle
												&& !(getEnv(enDessous) instanceof Bloc))
												|| getEnv(enDessous) instanceof bloc.BlocEchelle) {
											activerAnimation(helper.Images.climbLadder);
											getHeros().bas();
					...
					while (left && getHeros().estVivant() && !fini) {
						...
						if (getEnv(new jeu.Coordonnees(getHeros().getPosition().getX(),
								getHeros().getPosition().getY() + 1)) instanceof bloc.BlocGlu) {
							surGlu = true;
						} else {
							surGlu = false;
						}
						activerAnimation(helper.Images.moveLeft);
						...
						if (getHeros().getCentre().getX() > 15) {
							...
							if (getHeros().getPixels().getX() < getHeros().getPosition().getX() * 32) {
								jeu.Coordonnees aGauche = new jeu.Coordonnees(getHeros().getPosition().getX() - 1,
								...
								if (!(getEnv(aGauche) instanceof Bloc)) {
									 ...
									 * est concret
									 */
									if (!getEnv(aGauche).isConcret() || getEnv(aGauche) instanceof bloc.BlocBarreTraversee) {
										/*
					...
					while (up && getHeros().estVivant() && !fini) {
						...
						if (getHeros().getCentre().getY() > 16) {
							...
							if (getHeros().getPixels().getY() > getHeros().getPosition().getY() * 32) {
								...
								jeu.Coordonnees auDessus = new jeu.Coordonnees(getHeros().getPosition().getX(),
										getHeros().getPosition().getY() - 1);
								/*
								...
								if (getEnv(auDessus) instanceof Bloc && ((Bloc) getEnv(auDessus)).isConcret()) {
									 ...
									 */
								} else if (getEnv(getHeros().getPosition()) instanceof bloc.BlocEchelle) {
									getHeros().haut();
								} else {
									setUp(false);
									getHeros().setMouvement(false);
					...
					while (right && getHeros().estVivant() && !fini) {
						...
						activerAnimation(helper.Images.moveRight);
						/* Gestion de la collision a droite */

						if (getHeros().getCentre().getX() < 945) {
							/* On regarde si le heros est "dans" la case */
							if ((getHeros().getPixels().getX()) < getHeros().getPosition().getX() * 32) {
								jeu.Coordonnees aDroite = new jeu.Coordonnees(getHeros().getPosition().getX() + 1,
										getHeros().getPosition().getY());
								...
								if (!(getEnv(aDroite) instanceof Bloc) || ((getEnv(aDroite) instanceof Bloc)
									...
									if (!(getEnv(aDroite).isConcret())
											|| getEnv(aDroite) instanceof bloc.BlocBarreTraversee) {
										if (surGlu) {
											getHeros().droite(getHeros().getMoveSpeed() / 2);
										} else {
											getHeros().droite();
...
//			System.out.println(fini);
			...
			if (!fini) {
				...
				jeu.Coordonnees c = new jeu.Coordonnees(getHeros().getPixels().getX(), getHeros().getPixels().getY() - 32);
				getHeros().setMouvement(false);
				getHeros().getSprite().setPosition(c.getX(), c.getY());
				if (getHeros().isRegardeAGauche()) {
					/* Animation a gauche */
					jeu.Positionnable.GESTION_ANIM.ajouterPositionnableAnime(getHeros(), helper.Images.deathRight);
				} else {
					jeu.Positionnable.GESTION_ANIM.ajouterPositionnableAnime(getHeros(), helper.Images.deathLeft);
				}
				...
				jeu.Positionnable.GESTION_ANIM.ajouterPositionnableAnime(getHeros(), helper.Images.win);
				try {
					Thread.sleep(6000);
				} catch (InterruptedException e) {
					// TODO Auto-generated catch block
		...
		private void detecterObjet(jeu.Coordonnees posHeros) {
			/* On test si le joueur ne se trouve pas sur un lingot */
			item.Item item = getObj(posHeros);
			if (item instanceof item.Lingot) {
				/* On incremente le score du joueur */
				Heros h = (Heros) getHeros();
				h.augmenterScore(((item.Lingot) item).getValeur());
				...
				item.destruction();
			} else if (item instanceof item.PetiteBombe) {
				if (!((item.PetiteBombe) item).isActif()) {
					Heros h = (Heros) getHeros();
					h.ramasser(item);
					matrice.removeItem(item);
					// item.destruction();
		...
		private void testerSiFin() {
			...
			if (getEnv(getHeros().getPosition()) instanceof bloc.BlocPortailFinNiveau) {
				fini = true;

 ...
 *
 */
public class MadMonk extends personnage.Personnage {

	/* ***** ***** ATTRIBUTS ***** ***** */
	/** invLingot */
	private Lingot invLingot;
	/** cerveau Intelligence du MadMonk */
	private personnage.IAMadMonk cerveau;
	/** carte Matrice de jeu dans laquelle le MadMonk evolue */
	private Carte carte;
	...
	private personnage.Heros cible;
	/**
	...
	public MadMonk(Coordonnees position, String img, int vitesse, Carte terrain, int difficulte) {
            ...
            this.cerveau = new personnage.IANaive(position);
            if (difficulte == 2) {
	 ...
	 */
	public void cibler(personnage.Personnage personnage) {
            this.cible = (personnage.Heros) personnage;
	...
	private void chuter() {
            ...
            while (!(carte.getBloc(getPosition()) instanceof BlocEchelle) 
                ...
                /* on tombe */
                if (!isMouvement()) {
                    getSprite().playSequence(helper.ImagesMadMonk.fall, true);
                    setMouvement(true);
                }
                bas();
                pauseThread(20);
	 ...
	 */
	private void poursuivre(personnage.Heros player) {
            /*
	...
	private void tomber() {
            ...
            if ((this.carte.getBloc(enDessous) instanceof BlocTerre) && (!carte.getBloc(enDessous).isConcret())) {
                ...
                while ((getPixels().getY()) > (auDessus.getY() * 32)) {
                    /* on remonte */
                    if (!isMouvement()) {
                        getSprite().playSequence(helper.ImagesMadMonk.climbLadder, true);
                        setMouvement(true);
                    }
                    haut();
                    pauseThread(20);
...
//                  System.out.println("lolol3");
                    ...
                    if ((!(carte.getBloc(gauche) instanceof Bloc)) 
                         ...
                         */
                        getSprite().playSequence(helper.ImagesMadMonk.moveLeft, true);
...
//                    System.out.println("lolol4");
                    ...
                    if ((!(carte.getBloc(droite) instanceof Bloc)) 
                         ...
                         */
                        getSprite().playSequence(helper.ImagesMadMonk.moveRight, true);
